# coding=utf-8
# @FileName:img_resnet_20231020.py
# @Time:2023/10/20 
# @Author: CZH
import torchvision
import torch
...
from AF_SA_with_CA import AFCA_IN_AFSA
...
class ResNet50(nn.Module):
    def __init__(self, config, drop=0., **kwargs,):
        ...
        self.osa1 = _OSA_module(in_ch=1024, stage_ch=1024, concat_ch=1024, layer_per_block=6, module_name=3)
        self.osa2 = _OSA_module(in_ch=1024, stage_ch=1024, concat_ch=1024, layer_per_block=6, module_name=3)
        self.afsa_with_afca1 = AFCA_IN_AFSA(dim=1024, num_heads=8, height=24, weight=12)
        self.afsa_with_afca2 = AFCA_IN_AFSA(dim=1024, num_heads=8, height=24, weight=12)
    ...
    def forward(self, x):
        ...
        # f_cont_map = self.osa1(f_cont)
        # f_clo = self.afsa_with_afca(f_clo)
        # f_unclo = self.afsa_with_afca2(f_unclo)

        f_cont_map = self.osa1(f_cont)
        f_clo = self.osa2(f_clo)
        f_unclo = self.afsa_with_afca2(f_unclo)
