...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def forward(self, x, tensor_x, H, W):
        """
        Input: x: (B, H*W, C), H, W
        Output: x: (B, H*W, C)
        """
...
class AFCA_CI(nn.Module):
    def __init__(self, dim, num_heads, height, weight):
        super().__init__()
        self.dim = dim
        self.height = height
        self.weight = weight
        self.attention = Adaptive_Channel_Attention(dim=dim, num_heads=num_heads)

    def forward(self, x):
        batchsize, channel, height, weight = x.size()
        before_attention = x.permute(0, 2, 3, 1).contiguous().view(batchsize, -1, channel)
        after_attention = self.attention(before_attention, self.height, self.weight)
        output_x = after_attention.permute(0, 2, 1).contiguous().view(batchsize, channel, self.height, self.weight)
        return output_x

class AFCA_IN_AFSA(nn.Module):
    def __init__(self, dim, num_heads, height, weight):
        super().__init__()
        self.dim = dim
        self.height = height
        self.weight = weight
        self.ca_attention = Adaptive_Channel_Attention(dim=dim, num_heads=num_heads)
        self.sa_attention = Adaptive_Spatial_Attention(dim=dim, num_heads=num_heads)

    def forward(self, x):
        batchsize, channel, height, weight = x.size()
        before_attention = x.permute(0, 2, 3, 1).contiguous().view(batchsize, -1, channel)
        after_attention = self.ca_attention(before_attention, self.height, self.weight)
        output_x = after_attention.permute(0, 2, 1).contiguous().view(batchsize, channel, self.height, self.weight)

        sa_attention_before_attention = x.permute(0, 2, 3, 1).contiguous().view(batchsize, -1, channel)
        sa_after_attention = self.sa_attention(sa_attention_before_attention, output_x, self.height, self.weight)
        output_x = sa_after_attention.permute(0, 2, 1).contiguous().view(batchsize, channel, self.height, self.weight)
        return output_x
...
if __name__ == '__main__':
    ...
    model = AFCA_IN_AFSA(dim=1024, num_heads=8, height=24, weight=12)
    output = model(input)
