# coding=utf-8
# @FileName:img_resnet_3attn_tbb.py
# @Time:2023/10/16 
# @Author: CZH
...
class TBB(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = torch.mean(token, dim=1, keepdim=True)
        broadcast = torch.cat([mean] * (H*W), dim=1)
        x = token + broadcast
...
class ResNet50(nn.Module):
    def __init__(self, config, drop=0., **kwargs, ):
        ...
        self.sftca = SFTCA(channel=512, h=24, w=12)
        self.sftca1 = SFTCA(channel=1024, h=24, w=12)
    ...
    def forward(self, x):
        ...
        f_clo_sftca = self.tbb(f_clo_sftca)
