# coding=utf-8
# @FileName:img_resnet_without_cont_cbb.py
# @Time:2023/10/17 
# @Author: CZH

# coding=utf-8
# @FileName:img_resnet_without_cont.py
# @Time:2023/10/16
# @Author: CZH
...
class ResNet50(nn.Module):
    def __init__(self, config, drop=0., **kwargs, ):
        ...
        self.uncloth_net = GEN(in_feat_dim=1024, out_img_dim=1, config=config)
        # self.contour_net = GEN(in_feat_dim=self.contour_dim + self.cloth_dim, out_img_dim=1, config=config)
        self.cloth_net = GEN(in_feat_dim=1024, out_img_dim=1, config=config)
    ...
    def forward(self, x):
        ...
        f_unclo = x_ori[:, 0:1024, :, :]  # 0~512
        # f_cont = x_ori[:, self.uncloth_dim:self.uncloth_dim + self.contour_dim + self.cloth_dim, :, :] # 512~2048
        # f_clo = x_ori[:, self.uncloth_dim + self.contour_dim:self.uncloth_dim + self.contour_dim + self.cloth_dim, :, :] # 1536~2048

        f_clo = x_ori[:, 1024:2048, :, :]

        # print("f_unclo.shape:", f_unclo.shape)
        # print("f_clo.shape", f_clo.shape)

        # print("x_ori.shape:", x_ori.shape)
        # print("self.uncloth_dim:", self.uncloth_dim) # 1024
        # print("self.contour_dim:", self.contour_dim) # 512
        # print("self.cloth_dim:", self.cloth_dim) # 512
        # print("self.uncloth_dim + self.contour_dim", self.uncloth_dim + self.contour_dim) # 1536
        # print("self.uncloth_dim + self.contour_dim + self.cloth_dim", self.uncloth_dim + self.contour_dim + self.cloth_dim) # 2048

        f_unclo_local = self.local(f_unclo)
        f_clo_sftca = self.sftca(f_clo)
        ...
        conv_x_2 = self.dwconv2(f_clo)
        map2_1 = self.channel_interaction2(conv_x_2)
        map2_2 = self.spatial_interaction2(f_clo_sftca)
        sftca_map = f_clo_sftca * torch.sigmoid(map2_1)
        f_clo_map_ = conv_x_2 * torch.sigmoid(map2_2)
        f_clo_map = f_clo_map_ + sftca_map
